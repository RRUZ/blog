    ...
    begin
      ...
      Case rule.Protocol of
        NET_FW_IP_PROTOCOL_TCP:
          Writeln('  IP Protocol:        TCP.');
        NET_FW_IP_PROTOCOL_UDP:
          Writeln('  IP Protocol:        UDP.');
        NET_FW_IP_PROTOCOL_ICMPv4:
          Writeln('  IP Protocol:        UDP.');
        NET_FW_IP_PROTOCOL_ICMPv6:
          Writeln('  IP Protocol:        UDP.');
      Else
        Writeln('  IP Protocol:        ' + VarToStr(rule.Protocol));
      End;

      if (rule.Protocol = NET_FW_IP_PROTOCOL_TCP) or
      ...
      Case rule.Direction of
        NET_FW_RULE_DIR_IN:
          Writeln('  Direction:          In');
        NET_FW_RULE_DIR_OUT:
          Writeln('  Direction:          Out');
      End;
      ...
      Case rule.Action of
        NET_FW_ACTION_ALLOW:
          Writeln('  Action:             Allow');
        NET_FW_ACTION_BLOCK:
          Writeln('  Action:             Block');
      End;
